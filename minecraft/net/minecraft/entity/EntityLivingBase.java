...
public abstract class EntityLivingBase extends Entity {
    ...
    private int getArmSwingAnimationEnd() {
    	if(Killaura.instance.hasTarget()&& Management.instance.settingsmgr.getSettingByName("Killaura").getItemByName("FakeBlock").isState() || Management.instance.modulemgr.getModuleByName("Scaffold").isEnabled()) {
			return this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 20);
		}else{
     ...
     */
    /*
     * protected void jump() { this.motionY = (double) this.getJumpUpwardsMotion();
     * 
     * if (this.isPotionActive(Potion.jump)) { this.motionY += (double) ((float)
     * (this.getActivePotionEffect(Potion.jump).getAmplifier() + 1) * 0.1F); }
     * 
     * if (this.isSprinting()) { float f = this.rotationYaw * 0.017453292F;
     * this.motionX -= (double) (MathHelper.sin(f) * 0.2F); this.motionZ += (double)
    ...
    /*
     * public Vec3 getLook(float partialTicks) { //MouseDelayFix
     * https://github.com/prplz/MouseDelayFix if (this instanceof EntityPlayerSP) {
     * return super.getLook(partialTicks); } if (partialTicks == 1.0F) { return
     * this.getVectorForRotation(this.rotationPitch, this.rotationYawHead); } else {
     * float f = this.prevRotationPitch + (this.rotationPitch -
     * this.prevRotationPitch) * partialTicks; float f1 = this.prevRotationYawHead +
     * (this.rotationYawHead - this.prevRotationYawHead) * partialTicks; return
     * this.getVectorForRotation(f, f1); } }
     */
