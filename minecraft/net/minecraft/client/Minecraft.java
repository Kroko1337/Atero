...
import de.LCA_MODZ.discord.DiscordRPCHandler;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        DiscordRPCHandler.instance.init();
    ...
    private void createDisplay() throws LWJGLException {
        ...
        Display.setTitle(Management.instance.getTitle());
    ...
    // Draws the Minecraft Start Splash Screen
    private void drawSplashScreen(TextureManager textureManagerInstance) throws LWJGLException {
        System.out.println("Draw");
        /*
         * ScaledResolution scaledresolution = new ScaledResolution(this); int i =
         * scaledresolution.getScaleFactor(); Framebuffer framebuffer = new
         * Framebuffer(scaledresolution.getScaledWidth() * i,
         * scaledresolution.getScaledHeight() * i, true);
         * framebuffer.bindFramebuffer(false); GlStateManager.matrixMode(5889);
         * GlStateManager.loadIdentity(); GlStateManager.ortho(0.0D,
         * (double)scaledresolution.getScaledWidth(),
         ...
         * GlStateManager.disableLighting(); GlStateManager.disableFog();
         ...
         * GlStateManager.translate(0.0F, 0.0F, -2000.0F);
         * GlStateManager.disableLighting(); GlStateManager.disableFog();
         * GlStateManager.disableDepth(); GlStateManager.enableTexture2D(); InputStream
         * inputstream = null;
         *
         * try { inputstream =
         * this.mcDefaultResourcePack.getInputStream(locationMojangPng); this.mojangLogo
         * = textureManagerInstance.getDynamicTextureLocation("logo", new
         * DynamicTexture(ImageIO.read(inputstream)));
         * textureManagerInstance.bindTexture(this.mojangLogo); } catch (IOException
         * ioexception) { logger.error((String)("Unable to load logo: " +
         * locationMojangPng), (Throwable)ioexception); } finally {
         * IOUtils.closeQuietly(inputstream); }
         *
         * Tessellator tessellator = Tessellator.getInstance(); WorldRenderer
         * worldrenderer = tessellator.getWorldRenderer(); worldrenderer.begin(7,
         * DefaultVertexFormats.POSITION_TEX_COLOR); worldrenderer.pos(0.0D,
         * (double)this.displayHeight, 0.0D).tex(0.0D, 0.0D).color(255, 255, 255,
         * 255).endVertex(); worldrenderer.pos((double)this.displayWidth,
         * (double)this.displayHeight, 0.0D).tex(0.0D, 0.0D).color(255, 255, 255,
         * 255).endVertex(); worldrenderer.pos((double)this.displayWidth, 0.0D,
         * 0.0D).tex(0.0D, 0.0D).color(255, 255, 255, 255).endVertex();
         * worldrenderer.pos(0.0D, 0.0D, 0.0D).tex(0.0D, 0.0D).color(255, 255, 255,
         * 255).endVertex(); tessellator.draw(); GlStateManager.color(1.0F, 1.0F, 1.0F,
         * 1.0F); int j = 256; int k = 256;
         * this.func_181536_a((scaledresolution.getScaledWidth() - j) / 2,
         * (scaledresolution.getScaledHeight() - k) / 2, 0, 0, j, k, 255, 255, 255,
         * 255); GlStateManager.disableLighting(); GlStateManager.disableFog();
         * framebuffer.unbindFramebuffer();
         * framebuffer.framebufferRender(scaledresolution.getScaledWidth() * i,
         * scaledresolution.getScaledHeight() * i); GlStateManager.enableAlpha();
         * GlStateManager.alphaFunc(516, 0.1F);
         */
        // this.updateDisplay();
    }
    ...
    private void runGameLoop() throws IOException {
        ...
        EventRenderShader render = new EventRenderShader();
        EventManager.call(render);
    ...
    public void dispatchKeypresses() {
        ...
        if (key != 0 && !Keyboard.isRepeatEvent()) {
            if (!(this.currentScreen instanceof GuiControls)
                ...
                if (Keyboard.getEventKeyState()) {
                    if (key == this.gameSettings.keyBindStreamStartStop.getKeyCode()) {
                    ...
                    } else if (key == this.gameSettings.keyBindStreamPauseUnpause.getKeyCode()) {
                        if (this.getTwitchStream().isBroadcasting()) {
                    ...
                    } else if (key == this.gameSettings.keyBindStreamCommercials.getKeyCode()) {
                        if (this.getTwitchStream().isBroadcasting()) {
