...
public class ColorPicker {
    ...
    public void draw(int x, int y, int width, int height, int mouseX, int mouseY, Color currentColor) {
        ...
        for (int i = 0; i < height; i++) {
            drawRect(x + width + 10, (int) (y + (h * i)), x + width + 20, (int) (y + (h * (i + 1))), Color.HSBtoRGB((float) i / height, 1, 1));

            if (Mouse.isButtonDown(0) && mouseX >= x + width + 10 && mouseX <= x + width + 20 && mouseY >= y + (h * i) && mouseY <= y + (h * (i + 1))) {
        ...
        for (int i = 0; i < height; i++) {
            if (color == Color.HSBtoRGB((float) i / height, 1, 1)) {
                drawRect(x + width + 10, (int) (y + (h * i) + 1), x + width + 20, (int) (y + (h * (i + 1)) + 2), Color.black.getRGB());
                drawRect(x + width + 10, (int) (y + (h * i) - 2), x + width + 20, (int)(y + (h * (i + 1)) - 1), Color.black.getRGB());
            }
        }
        drawRect(x, y + height + 5, x + width / 2, y + height + 25, currentColor.getRGB());
        GL11.glColor3d(1,1,1);
        fr.drawString("#" + Integer.toHexString(currentColor.getRGB()).substring(2), x + width / 2 + 5, y + height + 30 / 2 - fr.FONT_HEIGHT / 2, -1);
